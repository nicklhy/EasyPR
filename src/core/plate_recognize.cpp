...
// !车牌识别模块
int CPlateRecognize::plateRecognize(Mat src, std::vector<string> &licenseVec, std::vector<CPlate> &plateVec, int index) {
  // 车牌方块集合
  ...
  if (resultPD == 0) {
    ...
    licenseVec = std::vector<std::string>(num, "");
    int i = 0;
